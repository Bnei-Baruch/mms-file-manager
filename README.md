...
- **Middleware** support with [Negroni](https://github.com/codegangsta/negroni). The template has [a single custom middleware handler](https://github.com/Bnei-Baruch/mms-file-manager/blob/master/config/assets.go#L14-L49) to serve as inspiration, but [more can be added](https://github.com/Bnei-Baruch/mms-file-manager/blob/master/config/app.go#L46-L51) easily if needed.
...
- **Database ORM** support with [Gorm](https://github.com/jinzhu/gorm). The template ships with a [simple Go struct](https://github.com/Bnei-Baruch/mms-file-manager/blob/master/models/post.go) and some [basic CRUD routes](https://github.com/Bnei-Baruch/mms-file-manager/blob/master/routes/posts.go).
...
- **Asset pipeline** with [Gulp](http://gulpjs.com/). Although not specifically a Golang setup, it generates digested `.coffee` and `.scss`  assets, as well as a `manifest.json` with the file paths. A Golang [`asset_path`](https://github.com/Bnei-Baruch/mms-file-manager/blob/master/templates/layouts/layout.html#L4-L5) helper is available in the templates. Digested assets are disabled in development mode for ease of development.
...
- **Testing** with [Ginkgo](http://onsi.github.io/ginkgo/) and [Gomega](http://onsi.github.io/gomega/). Golang prefers the tests to live alongside the actual files, and will automatically ignore files named `*_test.go` during compilation. This template has both simple [route tests](https://github.com/Bnei-Baruch/mms-file-manager/blob/master/routes/posts_test.go) and [model tests](https://github.com/Bnei-Baruch/mms-file-manager/blob/master/models/post_test.go), as well as some nice helper functions to access the HTTP responses like you know it from `RSpec`.
...
2. Clone this repo to your `GOPATH/src` folder. `git clone git@github.com:Bnei-Baruch/mms-file-manager.git github.com/Bnei-Baruch/mms-file-manager`
...
That's it. If you start using this template for your own application, you might want to rename the `Bnei-Baruch/mms-file-manager` packages to your own repository name.
